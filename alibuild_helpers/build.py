...
def createDistLinks(spec, specs, args, repoType, requiresType):
  ...
  if args.writeStore.startswith("s3://"):
    ...
    cmd = format("set -x; cd %(w)s && "
                 ...
                 "  echo $HASHEDURL | s3cmd put --skip-existing -q -P -s --add-header=\"x-amz-website-redirect-location:https://s3.cern.ch/swift/v1/%(b)s/${HASHEDURL}\" --host s3.cern.ch --host-bucket %(b)s.s3.cern.ch - s3://%(b)s/$x 2>&1;"
                 "done",
                 w=args.workDir,
                 b=bucket,
                 t=target)
