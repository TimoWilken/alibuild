...
class Boto3RemoteSync:
  ...
  def __init__(self, remoteStore, writeStore, architecture, workdir):
    ...
    self.readBucket = re.sub("^b3://", "", remoteStore)
  ...
  def _s3_listdir(self, dirname):
    ...
    pages = self.s3.get_paginator("list_objects_v2") \
                   .paginate(Bucket=self.readBucket, Delimiter="/",
  ...
  def syncToLocal(self, p, spec):
    ...
    if glob.glob(os.path.join(self.workdir, spec["remote_store_path"],
      ...
      for tarball in self._s3_listdir(spec["remote_store_path"]):
        ...
        self.s3.download_file(Bucket=self.readBucket, Key=tarball,
    ...
    for line in self.s3.get_object(
        Bucket=self.readBucket, Key=spec["remote_links_path"] + ".manifest"
    )["Body"].iter_lines():
      link_name, has_sep, target = line.partition(b"\t")
      if not has_sep:
      ...
      link_name = os.path.join(self.workdir, spec["remote_tar_link_dir"],
                               os.fsdecode(link_name))
      dieOnError(execute("ln -sf {} {}".format(target, link_name)),
    ...
    for link_key in self._s3_listdir(spec["remote_links_path"]):
      ...
      resp = self.s3.get_object(Bucket=self.readBucket, Key=link_key)
